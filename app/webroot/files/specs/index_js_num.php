...
class page extends Its_PageBase{
	...
	function main(){
		...
		if(is_ajax()){
			...
			$this->t->read("index_js_num.html");
		}

		$sid = Its_Request::get('sid');
		$ucd = Its_Request::get('ucd');

		$adn = Its_Request::get('adn');
		$show_ad_count = is_numeric($adn) ? ($adn - 1) : 0;
		...
		if($car->is_iPhone() || strpos(@$_SERVER['HTTP_USER_AGENT'], 'iPad') !== false){
...
// 		";
		...
		if($num + $pos == 0 || is_numeric($adn) && $adn == 0){
		...
		foreach($data as $c => $row){
			...
			if(is_numeric($adn) && $adn == 0) break;
			
			$this->t->assign('id', $row['id']);
			$this->t->assign('ad_title', mb_strimwidth($row['ad_title'],0,17,"…"));
			...
			$this->t->assign('ad_detail', mb_strimwidth($row['ad_detail'],0,28,"…"));
			...
			if($add_point > 0){
				...
				$this->t->assign('ad_point', (floor($add_point*10)/10));
			}

			$price = $row['ad_price'] == 0 ? '無料' : '有料'. $row['ad_price'] . '円';
