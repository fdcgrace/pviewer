<script>
	$(document).ready(function(){
		$('#archive').on('change',function(){
			if($(this).val() == 'getProjects')
			{
				window.location.href = 'viewArchive';
			}
			else
			{
			$.ajax({
...
<select id='archive'>
	...
	<option value='getLeaders'>Leaders</option>
	<option value='getMembers'>Members</option>
...
<table class='table table-hovered table-bordered table-striped content-panel'>
	...
	<thead>
							...
							<tbody>
								...
								<?php // miss ingna lang ko if nakay ganahan ipatabang or ipatest sa issue sa appli miss ?>	
								</tr>
